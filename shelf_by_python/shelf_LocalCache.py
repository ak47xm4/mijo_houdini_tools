...
networkDriverCache_path_expandString = hou.text.expandString(
    networkDriverCache_path)
...
for n in nodes:
    ...
    if node_type == '<hou.SopNodeType for Sop filecache::2.0>':
        ...
        filename_unexpandedString = n.parm(
            "file").unexpandedString()  # eval file cache path
        ...
        cache_files_dir = Path(
            filename).parent.absolute()  # get dir folder path
        ...
        cfn_split = cache_files_name.split(
            '.')  #cache_files_name_split_list    shortly
        ...
        if cache_files_name.endswith('.bgeo.sc'):
            # for xxxx.bgeo.sc
            goal_cache_filename = goal_cache_file_dir + cfn_split[
                0] + '.' + the_F_expression + '.bgeo.sc'
        else:
            goal_cache_filename = goal_cache_file_dir + cfn_split[
                0] + '.' + the_F_expression + '.' + cfn_split[-1]
